function [X] = NR_Reflect(X,sig,w,i,k,p,q,which)
% function [X] = NR_Reflect(X,sig,w,i,k,p,q,which)
% Apply a Householder reflection H(sig,w), embedded in rows i:k and columns i:k of an
% identiy matrix, to the matrix X, with {sig,w} as computed by NR_Reflect_Compute.m  
% INPUT:  X       = matrix to which reflection is to be applied
%         [sig,w] = parameters defining the rotation, as determined by NR_Reflect_Compute
...
% See also NR_Reflect_Compute. Verify with NR_Reflect_Test.
