function [A,p,r,v,R] = NR_GaussEchelon(A)
% function [A,p,r,v,R] = NR_GaussEchelon(A)
...
% an extension of NR_Gaussian elimination with partial pivoting.  The matrix A is replaced by
...
% Verify with <a href="matlab:help NR_GaussEchelonTest">NR_GaussEchelonTest</a>.
...
for j = 1:n,
   ...
   if amax > 0                                    % If it is nonzero, increment the rank,
      ...
      if r<m                                      % if not in the very last row, then
         ...
         end                                      % then perform the NR_Gauss transformation.
