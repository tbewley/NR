function [G,a,b,c] = NR_Thomas(a,b,c,G,n)
...
% See also NR_ThomasLU. Verify with NR_ThomasTest.

for j = 1:n-1,                               % FORWARD SWEEP
   a(j+1)   = - a(j+1) / b(j);               % This code is just a simplified version of
   b(j+1)   = b(j+1)   + a(j+1)*c(j);        % NR_Gauss.m (using a different notation for the
