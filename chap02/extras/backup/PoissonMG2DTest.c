...
#include <stdlib.h>            /* and Red/Black Gauss-Seidel (C syntax).                */
...
void poisson_mg(double **v, double **d, grid g, int n2, int n3);
void poisson_rb(double **v, double **d, grid g);
...
for (smooth=1; smooth<=n1; ++smooth) poisson_rb(v,d,g);
t0 = time(NULL); for (i=1; i<=15; ++i) {
   o = e; poisson_mg(v,d,g,n2,n3); e = max_error(v,d,g);
...
void poisson_mg(double **vf, double **df, grid gf, int n2, int n3) {
...
for (smooth=1;smooth<=n2;++smooth) poisson_rb(vf,df,gf);
...
if (gc.nx>3 && gc.ny>3 && (gf.nx % 4)==0 && (gf.ny % 4)==0)    /* Continue to even      */
   poisson_mg(vc,dc,gc,n2,n3);                                 /* coarser grid, or      */
else for (smooth=1;smooth<=20;++smooth) poisson_rb(vc,dc,gc);  /* solve coarsest system */
...
enforce_bcs(vf,gf);
for (smooth=1;smooth<=n3;++smooth) poisson_rb(vf,df,gf);
