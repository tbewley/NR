...
if ym==3; 
  ...
  if xm==3
     ...
     v{lev}(2:xmm,2)=NR_ThomasTT(-0.25,1,-0.25,v{lev}(2:xmm,2),xm-2);
  end
  EnforceBCs(lev);
else; for irb=0:1;
  ...
  elseif method=='z'                      % APPLY ZEBRA SMOOTHING.
    if direction==0;  
      if YBC==1
        ...
        v{lev}(2+irb:2:xmm,2:ymm)=(NR_ThomasTT(-0.25,1,-0.25,(v{lev}(2+irb:2:xmm,2:ymm))',ym-2))';
      else
      ...
      if XBC==1
        ...
        v{lev}(2:xmm,2+irb:2:ymm)=(NR_ThomasTT(-0.25,1,-0.25,v{lev}(2:xmm,2+irb:2:ymm),xmm-1)) ;
      else
        % fix the boundary conditions on this!
        a=-0.25*ones(ymm); b=ones(ymm); c=a;
        v{lev}(2:xmm,2+irb:2:ymm)=(v{lev}(2:xmm,(2+irb:2:ymm)+1)+v{lev}(2:xmm,(2+irb:2:ymm)-1))*0.25+d{lev}(2:xmm,(2+irb:2:ymm))
