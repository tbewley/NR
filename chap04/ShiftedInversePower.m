function [S,T] = NR_ShiftedInversePower(A,mu)
% function [S,T] = NR_ShiftedInversePower(A,mu)
...
% the NR_Schur vectors U(:,k) and upper-triangular T of the NR_Schur decomposition A=U*T*U'.
...
n=size(A,1);
for k=1:length(mu);
  B=A-mu(k)*eye(n);                                % Compute B=PLU (see NR_GaussPP.m) 
  ...
  if B(n,n)==0,              % NR_Eigenvalue exact! Solve Bs=0 exactly for a solution.
    S(n,k)=1;
    for i = n-1:-1:1,   S(i,k) = -B(i,i+1:n)*S(i+1:n,k) / B(i,i); end  % Backsubstitution.
  else                       % NR_Eigenvalue approximate. Apply Shifted Inverse Power method.
    S(:,k)=ones(n,1);        % Initialize (see footnote 8). (no need to apply P to e!)
    S(n,k)=S(n,k) / B(n,n);  % Solve Ux=e (see NR_GaussPP.m)
