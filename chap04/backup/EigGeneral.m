function [lam] = NR_EigGeneral(A)
% function [lam] = NR_EigGeneral(A)
% Computes the eigenvalues of a general complex matrix A. After an
% initial NR_Hessenberg decomposition, several explicitly shifted QR steps are taken.
...
while q>1        % Note: diagonal of T_22 block extends from {p,p} to {q,q} elements of T.
  ...
  for i=p:q-1                       % Apply same algorithm as in NR_QRFastGivensHessenberg.
