function [U,T]=NR_ReorderSchur(U,T,type,e)                % Numerical Renaissance Codebase 1.0
% This function reorders a NR_Schur decomposition such that the stable eigenvalues appear in
...
switch type(1)
  ...
  case 'a'     % type = 'absolute', to order by the absolute value of the real part
    for i=n-1:-1:1, a=i+1; b=n;      % (see NR_InsertionSort.m for details)
...
X=NR_Sylvester(T11,T22,T12,g,m,p);  [R,Q]=QRHouseholder([-X;g*eye(size(X,2),size(X,2))]);
if m>1                                                     % make new T22 upper triangular
  Q11=Q(1:m,1:p); Q12=Q(1:m,p+1:p+m); Q22=Q(m+1:m+p,p+1:p+m);  R11=R(1:p,1:p);
  [V,temp]=NR_Schur(Q12'*T11*(Q12-Q11*R11*Q22/g));
