function [U,T]=SchurSort(U,T)                        % Numerical Renaissance Codebase 1.0
% This function reorders a Schur decomposition such that the LHP eigenvalues appear in the
...
while p1>0  % start from bottom right and work towards upper left
    ...
    X=Sylvester(T11,T12,T22,g);   % T12 T22 now Swapped!!!???
    [Q,R]=QRHouseholder([-X;g*eye(size(X,2),size(X,2))]);
    
    % if length(T11)>1, use one more Schur decomposition to make new T22 upper triangular
    if length(T11)>1
        [m,n]=size(X);
