% script <a href="matlab:NR_ChebyshevTest">NR_ChebyshevTest</a>
% Test <a href="matlab:help NR_Chebyshev">NR_Chebyshev</a> by plotting the Chebyshev function and its derivatives
...
clear, for n=5:-1:1
  ...
  for i=0:250
    x(i+1)=-1.25+i*.01;
    T(i+1)  =NR_Chebyshev(n,x(i+1),0,1);
    Tp(i+1) =NR_Chebyshev(n,x(i+1),1,1);
    Tpp(i+1)=NR_Chebyshev(n,x(i+1),2,1);
  end
  plot(x,T,'k-',x,Tp,'r-',x,Tpp,'b-'); grid; axis tight
end
% end script NR_ChebyshevTest
