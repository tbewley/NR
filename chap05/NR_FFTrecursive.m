function [x] = NR_FFTrecursive(x,N,g)
% function [x] = NR_FFTrecursive(x,N,g)
...
% See also NR_FFTdirect, NR_FFTnonreordered. Depends on <a href="matlab:help NR_FFTrecursive">FFTrecursive</a> (that is, it's recursive...)
% Verify with: NR_FFTrecursiveTest.
                                                                                                       
if N>1                                        % If N=1, do nothing, else...
   M=N/2; w=exp(g*2*pi*i/N); xe=x(1:2:N-1); xo=x(2:2:N);
   xe=NR_FFTrecursive(xe,M,g);                % Compute FFTs of the even and odd parts.
   xo=NR_FFTrecursive(xo,M,g); 
   for j=1:M; xo(j,1)=w^(j-1)*xo(j,1); end    % Split each group in half and combine
   x=[xe+xo; xe-xo];                          % as in Eqn (5.25)
end
end % function NR_FFTrecursive
