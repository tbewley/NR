function [uhat]=NR_RFFT3D(u,NX,NY,NZ)
% function [uhat]=NR_RFFT3D(u,NX,NY,NZ)
...
for J=1:NY,   for K=1:NZ, uhat(:,J,K)=NR_RFFT(u(:,J,K),NX); end, end
uhat(1,:,:)=real(uhat(1,:,:)); 
for I=1:NX/2, for K=1:NZ, uhat(I,:,K)=NR_FFTdirect(uhat(I,:,K),NY,-1); end, end
uhat(:,NY/2+1,:)=0; 
if NZ>1;
  for I=1:NX/2, for J=1:NY, uhat(I,J,:)=NR_FFTdirect(uhat(I,J,:),NZ,-1); end, end
  uhat(:,:,NZ/2+1)=0; 
end
end % function NR_RFFT3D
