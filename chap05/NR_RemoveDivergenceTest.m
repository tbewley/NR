% script <a href="matlab:NR_RemoveDivergenceTest">NR_RemoveDivergenceTest</a>
% Test <a href="matlab:help NR_RemoveDivergence">NR_RemoveDivergence</a> with a randomly-generated 3D velocity field.
...
for i=1:2
  v1hat=NR_RFFT3D(v1(:,:,:),NX,NY,NZ);      % Transform v to Fourier space
  v2hat=NR_RFFT3D(v2(:,:,:),NX,NY,NZ);
  v3hat=NR_RFFT3D(v3(:,:,:),NX,NY,NZ);      % Now, remove the divergence.
  [u1hat,u2hat,u3hat]=NR_RemoveDivergence(v1hat,v2hat,v3hat,NX,NY,NZ,KX,KY,KZ);
  u1=NR_RFFT3Dinv(u1hat(:,:,:),NX,NY,NZ);   % Transform uhat back to Physical space
  u2=NR_RFFT3Dinv(u2hat(:,:,:),NX,NY,NZ);
  u3=NR_RFFT3Dinv(u3hat(:,:,:),NX,NY,NZ);   % u should now be divergence free.
  if i==1, disp('Divergence before and after pass 1:')
    v_divergence=NR_ComputeDivergence(v1hat,v2hat,v3hat,NX,NY,NZ,KX,KY,KZ)
    u_divergence=NR_ComputeDivergence(u1hat,u2hat,u3hat,NX,NY,NZ,KX,KY,KZ)
...
% end script NR_RemoveDivergenceTest
