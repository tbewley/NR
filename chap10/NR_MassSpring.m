function [qs,es,ts]=NR_MassSpring(method,Tmax,max_iterations)
% function [qs,energy]=NR_MassSpring(method,Tmax,max_iterations)
% Simulate the evolution of an undamped mass-spring oscillator using method=SI4 or method=RK4.
% SIMPLE TEST: clear; global m k, m=1; k=1; Tmax=(2*pi)*20; [qs,es,ts]=NR_MassSpring('RK4',Tmax,Tmax*1);
%              figure(1), plot(ts,qs,'k*-'), hold on, figure(2), plot(ts,es,'k*-'), hold on
%              [qs,es,ts]=NR_MassSpring('SI4',Tmax,Tmax*1);
...
end % function NR_MassSpringSimulation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
