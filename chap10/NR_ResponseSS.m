function NR_ResponseSS(A,B,C,D,w,g,Input)
% function NR_ResponseSS(A,B,C,D,w,g,Input)
...
% Depends on <a href="matlab:help NR_Gauss">NR_Gauss</a>, <a href="matlab:help NR_GaussLU">NR_GaussLU</a>.  Verify with: <a href="matlab:help NR_ResponseSSTest">NR_ResponseSSTest</a>.
...
for m=1:M
  ...
  Am=eye(N)-A*h/2; [x(:,2),Amod]=NR_Gauss(Am,Ap*x(:,1)+Bs*(u(:,2)+u(:,1)),N);
  for k=3:K+1, [x(:,k)]=NR_GaussLU(Amod,Ap*x(:,k-1)+Bs*(u(:,k)+u(:,k-1)),N); end
