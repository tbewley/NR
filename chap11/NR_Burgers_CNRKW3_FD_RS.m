function NR_Burgers_CNRKW3_FD_RS
% function <a href="matlab:NR_Burgers_CNRKW3_FD_RS">NR_Burgers_CNRKW3_FD_RS</a>
...
% See <a href="matlab:NRweb">Numerical Renaissance: simulation, optimization, & control</a>, Section 11.2.1.1.
...
% See also NR_Burgers_CNRKW3_FD, with 3 registers & 55 flops/timestep (instead of 2 & 57),
% and NR_Burgers_KS_CNRKW3_PS with a pseudospectral implementation.

%%%%%%%%%%%%%%%%%%%% Initialize the simulation paramters (user input) %%%%%%%%%%%%%%%%%%%%
clear all;  L=100;  Tmax=50;  N=100;  dt=0.5; PlotInterval=1;  dx=L/N;
x=(0:N)*dx;  y=-sin(pi*x/L)-sin(2*pi*x/L)+sin(6*pi*x/L);  NR_PlotXY(x,y,0,0,L,-3,3);
...
% (... initialization identical to that in NR_Burgers_RKW3CN_FD ...)
for k=1:Tmax/dt                                        
   %%%%%%%%%%%%%%%%%%%%%%%% FIRST RK STEP %%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   y(2:N)=NR_ThomasTT(a(1),b(1),c(1),y(2:N)',N-1);                                % ~ 8N
   %%%%%%%%%%%%%%%%%%%%%%%% SECOND RK STEP %%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   z(2:N)=NR_ThomasTT(a(2),b(2),c(2),z(2:N)',N-1);  z(N+1)=0;                     % ~ 8N
   %%%%%%%%%%%%%%%%%%%%%%%% THIRD RK STEP %%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   y(2:N)=NR_ThomasTT(a(3),b(3),c(3),y(2:N)',N-1);                                % ~ 8N
   if (mod(k,PlotInterval)==0) NR_PlotXY(x,y,k*dt,0,L,-3,3); end            
   %%%%%%%%%%%%%%%%%%%%%%%% END OF RK LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%   --------------
end                                                                     % Total: ~ 57N
end % function NR_Burgers_CNRKW3_FD_RS
