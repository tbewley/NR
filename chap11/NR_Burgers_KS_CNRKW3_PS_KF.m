...
dx=L/N; x=(0:N-1)'*dx; u =0.15*randn(N,1); uhat =NR_RFFT(u,N);
                       ub=0.15*randn(N,1); ubhat=NR_RFFT(ub,N);
...
for k=1:Tmax/dt
  for rk=1:3 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ALL 3 RK SUBSTEPS %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    rb=NR_RFFTinv(ubhat,N); rb=-rb.*rb; rbhat=i*kx.*NR_RFFT(rb,N);   % Leading-order cost:
    if (rk==1)                                                       % 2 FFTs per RK step
...
end % function NR_Burgers_KS_CNRKW3_PS_KF
