program NR_poisson_mg_rb_3d
! A 3D Poisson solver on uniform mesh using Multigrid and Red/Black NR_Gauss-Seidel.
! By Thomas Bewley and Paolo Luchini
include 'NR_poisson_mg_rb_3d.header'
...
do i=1,N1; call NR_poisson_rb(0); end do 
call date_and_time(values=ta); t = ta(5)*3600 + ta(6)*60 + ta(7) + 0.001*ta(8) 
iter=1; do
   o=e; call NR_poisson_mg(0); e = max_error(0)
...
end program NR_poisson_mg_rb_3d
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
recursive subroutine NR_poisson_mg(lev)
real*8 :: sum, nudge
include 'NR_poisson_mg_rb_3d.header'
do i=1,N2; call NR_poisson_rb(lev); end do 
...
if (lev<nlev-1) then; call NR_poisson_mg(lev+1)     ! Continue to even coarser grid, or
else; do i=1,50; call NR_poisson_rb(nlev); end do   ! solve coarsest system (almost exactly)
end if 
...
end do; end do; end do    ! Note that the next call to NR_poisson_rb takes care of red points
call enforce_bcs(lev)
do i=1,N3; call NR_poisson_rb(lev); end do 
end subroutine NR_poisson_mg 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine NR_poisson_rb(lev)
include 'NR_poisson_mg_rb_3d.header'      ! Apply Red/Black NR_Gauss-Seidel, with L from Poisson
