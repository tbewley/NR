% script <a href="matlab:NR_SS2TFTest">NR_SS2TFTest</a>
...
disp('Initial continuous-time system {A,B,C,D}:'), NR_ShowSys(A,B,C,D); disp(' ')

disp('Reduce to a SISO problem, and compare with Matlab''s built-in function.')
[b_NR_SISO,a_NR]        =NR_SS2TF(A,B(:,1),C(1,:),D(1,1)), b_size_SISO=size(b_NR_SISO)
...
[b_NR_SIMO,a_NR]        =NR_SS2TF(A,B(:,1),C,D(:,1)), b_size_SIMO=size(b_NR_SIMO)
...
[b_NR_MISO,a_NR]=NR_SS2TF(A,B,C(1,:),D(1,:));  b_size_MISO=size(b_NR_MISO)
...
[b_NR_MIMO,a_NR]=NR_SS2TF(A,B,C,D);           b_size_MIMO=size(b_NR_MIMO)
for j=1:b_size_MIMO(2), disp(sprintf('b_NR_MIMO(:,%d,:)',j))
  for i=1:b_size_MIMO(1), disp(reshape(b_NR_MIMO(i,j,:),1,b_size_MIMO(3))), end
end, a_NR

disp(' '); disp('Next <a href="matlab:help NRchap20">NRchap20</a> demo: <a href="matlab:NR_MaxEnergyGrowthTest">NR_MaxEnergyGrowthTest</a>'), disp(' ')
% end script NR_SS2TFTest
