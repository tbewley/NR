function [thetamax,x0]=NR_MaxEnergyGrowth(A,Q,tau,MODE)
% function [thetamax,x0]=NR_MaxEnergyGrowth(A,Q,tau,[MODE])
...
% Verify with: <a href="matlab:help NR_MaxEnergyGrowthTest">NR_MaxEnergyGrowthTest</a>.

if nargin==3, MODE='CT'; end, Qhi=Inv(sqrtm(Q));
if MODE=='CT', Phi=NR_MatrixExponential(A,tau); else, Phi=A^tau; end
[lam,S]=NR_Eig(Qhi*Phi'*Q*Phi*Qhi,'h');
thetamax=lam(end); x0=Qhi*S(:,end); x0=x0/sqrt(x0'*Q*x0);
end % function NR_MaxEnergyGrowth