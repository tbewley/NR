function x=TFnorm(A,B,C,p,MODE)
% function x=TFnorm(A,B,C,[p],[MODE])
...
% Verify with: <a href="matlab:help TFnormTest">TFnormTest</a>.

if nargin<5, MODE='CT'; end, if nargin<4, p='2'; end
if p=='2', P=CtrbGrammian(A,B,MODE); x=sqrt(sum(diag(C*P*C')));
...
gamma=10^x;
if P.MODE=='CT'
  lambda=Eig([P.A P.S/gamma; -P.Q/gamma -P.A'],'r');
  f=-1; for i=1:P.n*2, if abs(real(lambda(i)))<1e-5, f=1; return, end, end
else
  lambda=Eig([P.A-P.S*P.Ai*P.Q/gamma^2 P.S*P.Ai/gamma; -P.Ai*P.Q/gamma P.Ai],'r');
