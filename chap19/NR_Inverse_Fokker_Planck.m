...
clear; close all; format compact; N=101, L=6, d=L/(N-1); x=[-L/2:d:L/2]; y=[-L/2:d:L/2]; verbose=false;
...
for i=1:N
    for j=1:N
        ...
        elseif i==1 & j==N; LAP(c,c)=-2; LAP(c,s)=3; LAP(c,ss)=-3; LAP(c,sss)=1; LAP(c,e)=3; LAP(c,ee)=-3; LAP(c,eee)=1; % (curvature extrapolation of edges)
        elseif i==N & j==1; LAP(c,c)=-2; LAP(c,n)=3; LAP(c,nn)=-3; LAP(c,nnn)=1; LAP(c,w)=3; LAP(c,ww)=-3; LAP(c,www)=1; 
        elseif i==N & j==N; LAP(c,c)=-2; LAP(c,s)=3; LAP(c,ss)=-3; LAP(c,sss)=1; LAP(c,w)=3; LAP(c,ww)=-3; LAP(c,www)=1;
        elseif i==1; LAP(c,c)=-1; LAP(c,n)=1; LAP(c,s)=1; LAP(c,e)=-2; LAP(c,ee)=1; % Edge BCs
...
LHS=LAP/d^2+g_dot_NAB/d;       % Build LHS matrix while applying constants
PHI=LHS\RHS;
for i=1:N, for j=1:N, c=i+(j-1)*N; phi_num(i,j)=PHI(c); end, end % rearrange PHI as matrix
ii=round(N+1)/2; jj=round(N+1)/2; phi_num=phi_num-phi_num(ii,jj)
